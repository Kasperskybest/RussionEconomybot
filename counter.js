...
function counter(message, client) {
  ...
  if (!message.author.bot && message.author.id === count.author) {
    message.delete();
    message.reply("чиний ээлж биш байна").then(m => m.delete(3000));
    return;
  }
  if (!message.author.bot && isNaN(message.content)) {
    message.delete();
    message.reply("энэ channel-д зөвхөн тоо л бичнэ").then(m => m.delete(3000));
    return;
  }
  if (!message.author.bot && parseInt(message.content) !== count.number + 1) {
    message.delete();
    message.reply(`дараагын тоог бич ${count.number + 1}`).then(m => m.delete(3000));
    return;
  }
  count = db.set(`counter_${message.guild.id}`, { number: count.number + 1, author: message.author.id });
  channel.setTopic(`Дараагын тоог бич ${count.number + 1}.`);
}
